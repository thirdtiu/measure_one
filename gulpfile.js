...
gulp.task('images', function() {
  var out = folder.dist + '_assets/images/';
  return gulp.src(folder.src + '_assets/images/**/*')
    .pipe(newer(out))
...
gulp.task('bootstrap-css', gulp.series('images', function() {
  ...
  var postCssOpts = [
  assets({ loadPaths: ['_assets/images/'] }),
...
// new watch method
var cssWatcher = gulp.watch(folder.src + '_assets/scss/**', gulp.series('css', 'bootstrap-css'));
cssWatcher.on('all', function(event) {
  console.log('CSS watch: ' + event);
});

//image changes
var imageWatcher = gulp.watch(folder.src + '_assets/images/**/*', gulp.parallel('images'));
